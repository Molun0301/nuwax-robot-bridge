...
try:
    ...
    # [Sim2Sim-MuJoCo task] Fallback when ros2_numpy is unavailable in runtime env.
    from sensor_msgs_py import point_cloud2 as _pc2

    class _FallbackPointCloud2:
        @staticmethod
        def get_xyz_points(points):
            if points is None or not hasattr(points, "dtype") or points.dtype.names is None:
                return np.zeros((0, 3), dtype=np.float32)
            required = ("x", "y", "z")
            if not all(name in points.dtype.names for name in required):
                return np.zeros((0, 3), dtype=np.float32)
            xyz = np.column_stack((points["x"], points["y"], points["z"]))
            return xyz.astype(np.float32, copy=False)

    class _FallbackRos2Numpy:
        point_cloud2 = _FallbackPointCloud2()
        ...
        def numpify(msg):
            ...
            rows_raw = list(_pc2.read_points(msg, field_names=field_names, skip_nans=False))
            dtype = [(name, np.float32) for name in field_names]
            if not rows_raw:
            ...
            for row in rows_raw:
                if hasattr(row, "dtype") and getattr(row.dtype, "names", None):
                    rows.append(tuple(float(row[name]) for name in field_names))
                elif isinstance(row, np.ndarray):
                    flat = row.reshape(-1)
                    rows.append(tuple(float(flat[idx]) for idx in range(min(len(field_names), flat.size))))
                elif isinstance(row, (list, tuple)):
                    rows.append(tuple(float(row[idx]) for idx in range(min(len(field_names), len(row)))))
                else:
                    # Unknown row format; keep length-consistent with zeros.
                    rows.append(tuple(0.0 for _ in field_names))

            out = np.empty((len(rows),), dtype=dtype)
            for idx, name in enumerate(field_names):
                out[name] = np.asarray([r[idx] if idx < len(r) else 0.0 for r in rows], dtype=np.float32)
            return out

    rnp = _FallbackRos2Numpy()
from sensor_msgs.msg import PointCloud2, Image, CameraInfo
from sensor_msgs_py import point_cloud2
try:
    from tf_transformations import quaternion_matrix
except Exception:
    # [Sim2Sim-MuJoCo task] Fallback when tf_transformations is unavailable.
...
class ElevationMappingNode(Node):
    def __init__(self):
        ...
        for _name, _default in (
            ...
            if not self.has_parameter(_name):
    ...
    def publish_map(self, key: str) -> None:
        ...
        # Publish the actual rolling-grid array center so downstream samplers use
        # geometry consistent with the current map buffer, not the instantaneous base pose.
        gm.info.pose.position.x = float(center[0])
        gm.info.pose.position.y = float(center[1])
        gm.info.pose.position.z = float(center[2])

        # GridMap is published in map-frame coordinates; keep orientation identity so
        # downstream local samplers treat the array axes as aligned with the map frame.
