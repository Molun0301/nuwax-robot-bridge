...
class ElevationMap:
    ...
    def __init__(self, param: Parameter):
        ...
        try:
            ...
            # Runtime fallback for setups without torch/chainer; keep traversability neutral (all traversable).
            print(f"[WARN] Traversability filter unavailable, fallback to neutral traversability map: {exc}")

            def _fallback_traversability(elevation_input):
                h, w = elevation_input.shape
                oh = max(int(h) - 6, 1)
                ow = max(int(w) - 6, 1)
                return cp.ones((1, 1, oh, ow), dtype=self.data_type)

            self.traversability_filter = _fallback_traversability
